...
public class secure extends AbstractJni implements IOResolver {
    ...
    secure()  {
        ...
        DalvikModule dm_libUserEnv = vm.loadLibrary(new File("unidbg-android/src/test/resources/PDD_8/libUserEnv.so"),true);
        dm_libUserEnv.callJNI_OnLoad(emulator);
        DalvikModule dm = vm.loadLibrary(new File("unidbg-android/src/test/resources/PDD_8/libpdd_secure.so"), true);
...
//        demo.nativeGenerate();
        System.out.println("call nativeGenerate2");
    ...
    public void nativeGenerate2() {
        ...
        Number number = module.callFunction(emulator, 0x247d, list.toArray())[0];
...
//                return vm.resolveClass("java/util/UUID").newObject(UUID.randomUUID());
                return vm.resolveClass("java/util/UUID").newObject("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx");
            }
